...
| ColorPipelineFunctionExpansionCampaign | Active bounded campaign. Slice 1 enabled and qualified Root Glow. Slice 2 gave all 39 shipped functions canonical typed ports. Slice 3 now routes all 17 exact identity compatibility rows through live typed-resolver authority while retaining seven signed/category/recipe-owned specialized routes with explicit reasons; the published report exposes the 17/7 split and public recipe/replay behavior remains green. Next: add and qualify the locked low-risk Shape/Palette/Grading batch. | See `docs/notes/color_pipeline_function_expansion_campaign_PHASED_PLAN.md`; composite functions, coordinate sources, gradient assets, and graph UI remain deferred to the mandatory post-campaign replan |

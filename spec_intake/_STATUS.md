...
| ColorPipelineFunctionExpansionCampaign | Active bounded campaign. Slice 1 enabled and qualified Root Glow through graph recipe authority using `root_log_proximity_v1`, `signed_unit_map_v1`, heatmap, and Contrast Lift; the weak dedicated Glow grader was removed from this recipe after measured sensitivity failure. Next: complete typed ports for existing functions, expand only safe live resolver routes, then add the locked low-risk Shape/Palette/Grading batch. | See `docs/notes/color_pipeline_function_expansion_campaign_PHASED_PLAN.md`; composite functions, coordinate sources, gradient assets, and graph UI remain deferred to the mandatory post-campaign replan |

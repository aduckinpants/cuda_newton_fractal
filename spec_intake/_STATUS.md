...
| SdfFieldPackNearTermPlanning | SDF modernization roadmap is partially shipped: Lens SDF control truth, scalar field authority, Lens semantics, authored SDF pack CPU reference, CUDA evaluator/hardening, GPU Lens SDF backend, flashlight/runtime-walk headless/report SDF signal consumption, live Color Pipeline SDF source rows, Capture Finding parity, normal viewport SDF overlay, SDF Source row customization, capture/replay authority, `sdf_normal_angle` phase-signal metadata, Color Pipeline fractal-switch preservation, SDF realtime pacing telemetry, SDF postprocess signal specialization, SDF preview postprocess quality policy, full-quality downsampled-field postprocess cell reuse, CUDA direct-scalar/field-signal SDF postprocess, live-only adaptive SDF field resolution, Lens Field v2 sign contrast, field-generation stage telemetry, CUDA JFA buffer reuse, repeated median SDF witness reporting, measured CUDA SDF postprocess scratch-buffer reuse, row-local SDF field downsample UI/runtime authority, SDF Normal Angle Beauty, authored SDF pack viewer UI, authored-pack Color Pipeline/overlay field consumption, the first selectable `sdf_pack_scene` lane shell with `sdf_smooth_lattice_2d`, the curated built-in SDF pack catalog seed, the SDF drag camera mapping repair, typed Source-row applicators, mixed SDF/non-SDF Source-row composition for supported renderer-backed lanes, Color Pipeline function picker/layout refinement, Capture Finding `fractal-state.json` review sidecars, published-runtime SDF pack staging, SDF field capability reporting, field-primary mixed Source denial proof, the field-primary `explaino_root_sdf` lane plus its control/seed-dynamics/generated-N-root repairs, and Wave 2 root-field consumers (`root_field_debug_views`, `explaino_mandelbrot_root_trap`, `explaino_magnet_root_well`) are done. The next SDF engine target is field-generation/downsample optimization before new SDF ops; full authoring UX, new SDF ops, recursive/apollonian packs, broader SDF-native families, and mixed Source rows on lanes without renderer-backed non-SDF signal producers remain deferred. | See `docs/notes/sdf_field_pack_near_term_TODO.md`, `docs/notes/sdf_engine_completion_sprint_PHASED_PLAN.md`, `docs/notes/sdf_next_five_work_campaign_PHASED_PLAN.md`, `docs/notes/sdf_step6_evidence_replan_PHASED_PLAN.md`, `docs/notes/sdf_postprocess_measured_optimization_PHASED_PLAN.md`, `docs/notes/sdf_field_generation_optimization_PHASED_PLAN.md`, `docs/notes/sdf_field_generation_optimization_preflight_PHASED_PLAN.md`, `docs/notes/sdf_realtime_pacing_telemetry_PHASED_PLAN.md`, `docs/notes/sdf_postprocess_signal_specialization_PHASED_PLAN.md`, `docs/notes/sdf_postprocess_truth_sync_PHASED_PLAN.md`, `docs/notes/sdf_field_quality_downsample_policy_PHASED_PLAN.md`, `docs/notes/color_pipeline_composition_preset_ux_review_PHASED_PLAN.md`, `docs/notes/color_pipeline_applicative_glue_mini_campaign_PHASED_PLAN.md`, and `docs/notes/root_field_consumer_wave2_PHASED_PLAN.md` |
| ColorPipelineUiSaltMetadataAuthority | UI-Salt materialized metadata owns the Color Pipeline catalog descriptors, compatibility lookup, companion suggestions, recipe expansion backend seams, preset workflow, function-library taxonomy/reporting, Source-row applicator metadata, mixed Source-row composition authority for supported renderer-backed lanes, and grouped function-picker presentation. The SDF composition runtime-authority repair on `08e62b6` closed the disabled-row compatibility leak and restored `sdf_curvature` blending with `sdf_normal_angle` in SDF-only Source stacks. Boundary-masked SDF Normal Angle Beauty, typed Source-row masks, mixed SDF/non-SDF Source rows, and function picker/layout refinement are shipped. Remaining broader follow-ups are additional applicator semantics, authored SDF pack catalog/authoring UX, and the later Salticid adapter/removal campaign after the vertical is stable. | See `docs/notes/color_pipeline_sdf_composition_runtime_authority_PHASED_PLAN.md`, `docs/notes/color_pipeline_preset_workflow_truth_PHASED_PLAN.md`, `docs/notes/color_pipeline_composition_preset_ux_review_PHASED_PLAN.md`, `docs/notes/color_pipeline_function_library_taxonomy_PHASED_PLAN.md`, `docs/notes/color_pipeline_function_picker_layout_PHASED_PLAN.md`, and the `docs/notes/ui_salt_*_PHASED_PLAN.md` backend seam plans |
| ColorPipelineFunctionExpansionCampaign | Shipped at `7c4c0a7`: Root Glow qualified, all 39 pre-batch functions have canonical typed ports, 17 exact routes use live typed-resolver authority, seven specialized routes remain explicit, and the ten-function Shape/Palette/Grading batch is runtime-backed and replay-proven. | See `docs/notes/color_pipeline_function_expansion_campaign_PHASED_PLAN.md`; the follow-up composite-function campaign is active under `docs/notes/color_pipeline_composite_function_v1_PHASED_PLAN.md` |
| ColorPipelineCompositeFunctionV1 | Shipped on the composite campaign branch: separate validated composite metadata, generic prepared draft transaction, expanded-primitive replay authority, truthful wrapper receipts, and the `unit_contours_v1` Shape wrapper with exact manual-stack/public-runtime parity. | See `docs/notes/color_pipeline_composite_function_v1_PHASED_PLAN.md`; preplanned composite work is exhausted pending replan |
...
| IfsEngineFoundationCampaign | Decision-complete deferred campaign for an ordinary affine IFS accumulation substrate, deterministic CPU/CUDA courts, headless runtime operation, viewer lane, and measured pacing. A mandatory pause separates that useful vertical from graph-directed IFS, IFS-to-SDF, Flame, and Science Mode work. | See `docs/notes/ifs_engine_foundation_campaign_PHASED_PLAN.md`; implementation is not active in the current docs/test-hardening slice |
| ModelDiagnosticsOverlayCapabilityCampaign | Decision-complete documentation-only campaign for catalog-wide model classification, typed coordinate spaces, provider receipts, bounded lifecycle, and a later diagnostics panel/live-overlay/annotated-capture vertical. Product implementation is paused until MD-0 through MD-6 establish meaningful capability coverage across all selectors. | See `docs/notes/model_diagnostics_overlay_capability_campaign_PHASED_PLAN.md`; next future slice is MD-0 catalog coverage court |
| ExplainOOverLegacyCampaign | Imported and repo-reviewed paired-evaluator campaign for exact pre-`h(t)` legacy recovery, step-aligned modern comparison, typed disagreement, and later interrogation. It is the next product campaign when engine work resumes, but Slice 0 is blocked until the historical source/build/state/image evidence is supplied and checksum-verified. | See `docs/notes/EXPLAINO_OVER_LEGACY_PROPOSAL.md` and `docs/notes/explaino_over_legacy_PHASED_PLAN.md`; no product implementation is active |

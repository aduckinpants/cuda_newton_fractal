...
| ColorPipelineFunctionExpansionCampaign | Active bounded campaign. Slice 1 enabled and qualified Root Glow. Slice 2 now gives all 39 shipped functions canonical typed ports while preserving the 6 typed-resolved / 18 runtime-override compatibility split, visible UI, public recipes, and capture/replay behavior. Next: expand only coherent live typed-resolver routes, then add the locked low-risk Shape/Palette/Grading batch. | See `docs/notes/color_pipeline_function_expansion_campaign_PHASED_PLAN.md`; composite functions, coordinate sources, gradient assets, and graph UI remain deferred to the mandatory post-campaign replan |

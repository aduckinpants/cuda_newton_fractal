...
| SdfFieldPackNearTermPlanning | SDF modernization roadmap is partially shipped: Lens SDF control truth, scalar field authority, Lens semantics, authored SDF pack CPU reference, CUDA evaluator/hardening, and flashlight/runtime-walk headless/report SDF signal consumption are done. Deferred: CUDA Lens SDF backend, live Color Pipeline SDF rows, viewport overlay, authored-pack UI/live viewport integration, and SDF-native fractal lanes. | See `docs/notes/sdf_field_pack_near_term_TODO.md` and `docs/notes/sdf_field_pack_near_term_planning_PHASED_PLAN.md` |

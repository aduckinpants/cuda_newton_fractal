...
| SdfFieldPackNearTermPlanning | SDF modernization roadmap is partially shipped: Lens SDF control truth, scalar field authority, Lens semantics, authored SDF pack CPU reference, CUDA evaluator/hardening, GPU Lens SDF backend, flashlight/runtime-walk headless/report SDF signal consumption, live Color Pipeline SDF source rows, Capture Finding parity, normal viewport SDF overlay, SDF Source row customization, capture/replay authority, and `sdf_normal_angle` phase-signal metadata are done. Forward SDF feature work is paused behind the Color Pipeline fractal-switch preservation regression and later Color Pipeline composition UX review. Deferred: authored-pack UI/live viewport integration and SDF-native fractal lanes. | See `docs/notes/sdf_field_pack_near_term_TODO.md` and `docs/notes/sdf_field_pack_near_term_planning_PHASED_PLAN.md` |

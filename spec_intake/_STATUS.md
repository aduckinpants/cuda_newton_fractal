...
| SdfFieldPackNearTermPlanning | SDF modernization roadmap is partially shipped: Lens SDF control truth, scalar field authority, Lens semantics, authored SDF pack CPU reference, CUDA evaluator/hardening, GPU Lens SDF backend, flashlight/runtime-walk headless/report SDF signal consumption, live Color Pipeline SDF source rows, Capture Finding parity, normal viewport SDF overlay, SDF Source row customization, capture/replay authority, `sdf_normal_angle` phase-signal metadata, Color Pipeline fractal-switch preservation, SDF realtime pacing telemetry, SDF postprocess signal specialization, SDF preview postprocess quality policy, and full-quality downsampled-field postprocess cell reuse are done. Remaining larger performance/design choices are per-row/multi-field SDF downsample authority or GPU Color Pipeline postprocess before authored-pack UI or SDF-native lanes. | See `docs/notes/sdf_field_pack_near_term_TODO.md`, `docs/notes/sdf_realtime_pacing_telemetry_PHASED_PLAN.md`, `docs/notes/sdf_postprocess_signal_specialization_PHASED_PLAN.md`, `docs/notes/sdf_postprocess_truth_sync_PHASED_PLAN.md`, `docs/notes/sdf_field_quality_downsample_policy_PHASED_PLAN.md`, and `docs/notes/color_pipeline_composition_preset_ux_review_PHASED_PLAN.md` |
| ColorPipelineUiSaltMetadataAuthority | UI-Salt materialized metadata owns the Color Pipeline catalog descriptors, compatibility lookup, companion suggestions, and recipe expansion backend seams through `d908c54`. Product-facing preset workflow truth is shipped, and the current composition UI cleanup slice removes remaining visible draft/live-bridge wording without changing the internal row-stack implementation. Immediate reported follow-up bugs are disabled rows still affecting draft-only compatibility/error reporting and `sdf_curvature` no longer blending with `sdf_normal_angle` in an SDF-only Source stack. Remaining broader follow-ups are boundary-masked phase source, SDF masks/gates, and function-library taxonomy/layout work. | See `docs/notes/color_pipeline_sdf_composition_bug_intake_PHASED_PLAN.md`, `docs/notes/color_pipeline_preset_workflow_truth_PHASED_PLAN.md`, `docs/notes/color_pipeline_composition_ui_cleanup_PHASED_PLAN.md`, and the `docs/notes/ui_salt_*_PHASED_PLAN.md` backend seam plans |

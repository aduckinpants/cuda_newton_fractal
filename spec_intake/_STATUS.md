...
| ColorPipelineUiSaltMetadataAuthority | UI-Salt materialized metadata now owns the Color Pipeline catalog descriptors, compatibility lookup, companion suggestions, and recipe expansion backend seams through `d908c54`. The active follow-up is product-facing preset workflow truth, not another backend authority switch. | See `docs/notes/color_pipeline_preset_workflow_truth_PHASED_PLAN.md` plus the `docs/notes/ui_salt_*_PHASED_PLAN.md` backend seam plans |

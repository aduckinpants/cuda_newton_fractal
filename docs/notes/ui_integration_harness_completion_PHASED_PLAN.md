...
Open the next smallest real uncovered seam after `generic_function_cpu_eval`, currently `diagnostics_capture.h`: keep rejecting fake line-count wins, and prioritize seams that either already have real behavior hidden in the wrong binary or expose a narrow deterministic behavior surface that can move the module-level audit again.

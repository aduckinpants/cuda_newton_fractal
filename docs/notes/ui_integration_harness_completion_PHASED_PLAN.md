...
Open the next smallest real uncovered seam after `ui_schema_grouping`: keep rejecting fake line-count wins, and prioritize seams that either already have real behavior hidden in the wrong binary or expose a narrow deterministic behavior surface that can move the module-level audit again.

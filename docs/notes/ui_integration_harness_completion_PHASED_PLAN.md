...
Open the next smallest real uncovered seam after `fractal_probe_contract`, currently `fractal_types.h`: keep rejecting fake line-count wins, and prioritize seams that either already have real behavior hidden in the wrong binary or expose a narrow deterministic behavior surface that can move the module-level audit again.

...
Open the next smallest real uncovered seam: start with `ui_app/src/ui_schema_grouping.h`, since `safe_mode_schema` is now closed, and keep rejecting fake line-count wins that do not move the user-facing coverage authority or the module-level audit.

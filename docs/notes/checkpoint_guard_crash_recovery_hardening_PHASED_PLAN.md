...
- GREEN follow-up: `UserPromptSubmit` no longer falsely blocks a fresh clean prompt just because the active locked contract changed after the current clean `HEAD` was already receipted under the prior slice; contract-proof prompt blocking now stays tied to `HEAD` advancement, and when it does fire it reports the contract-proof reason instead of pretending the validation receipt is missing.
...
- [done] Later follow-up defect found and repaired: `UserPromptSubmit` could still false-block a clean repo after a fresh session-start breadcrumb switched the active contract, because the prompt hook reused closure-time contract-proof logic even when the session had not advanced `HEAD`, and the message surfaced that as a missing validation receipt. The prompt path now skips that closure-only contract-proof gate until `HEAD` actually advances and reports contract-proof failures accurately when they do apply.

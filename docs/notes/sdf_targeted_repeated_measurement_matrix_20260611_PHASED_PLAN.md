...
- Witness-tool regression: `py -3.14 tools/viewer_host_run_logged_command.py --label sdf_witness_tool_pytest --log artifacts/logs/sdf_witness_tool_pytest.log --out-json artifacts/validation/sdf_witness_tool_pytest.json --heartbeat-seconds 30 --timeout-seconds 120 -- py -3.14 -m pytest tests/test_sdf_performance_witness_tool.py` passed after the viewport-clamped settle predicate repair.

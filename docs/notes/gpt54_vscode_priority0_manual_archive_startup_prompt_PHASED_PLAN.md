...
Phase 2 complete - the paste-ready startup prompt is landed, hostile-reviewed against the repo rules and the restart inventory, and ready for docs-slice validation and clean checkpoint closure.
...
- [x] Phase 2 - write the final paste-ready prompt with explicit anti-lie rules, exact bootstrap reads/commands, strict scope lock, hostile-review requirements, and closure chain; validate and checkpoint it cleanly
...
The controlling risk is not missing wording; it is accidental softening. A generic "be careful" startup prompt would fail the user ask because the entire point is to hard-code the distrust posture that future VS Code sessions must inherit. The startup prompt is only good if it is explicit about what counts as a lie, explicit that all agents are distrusted by default, explicit that the only active lane is the historical manual archive blocker, and explicit that hostile review must audit both implementation claims and summary claims for overstatement. Any softened language, multi-lane ambiguity, or workflow/product-state mixing would make the startup prompt unsafe as a bootstrap surface.
...
- The user's additional requirement is not discoverable from repo docs alone: the startup prompt must explicitly call lying what it is, must say any agent is assumed to be a liar until proven otherwise, and must keep hostile review in that frame.
...
- Prompt hardening: the final prompt now names the exact runtime proof commands from the manual inertial repair slice instead of referring to that rail abstractly, and it tells the future session exactly when to reuse versus revise the existing slice surfaces.
...
- [done] Pass 3 - clean reread the repaired prompt as a future-session bootstrap surface; no additional real defect found after checking anti-lie bluntness, single-lane scope lock, exact bootstrap commands, exact runtime witnesses, and closure-chain wording.
...
- [done] Clean reread result: no additional ambiguity or scope leak remained after the final pass; the repaired prompt keeps the anti-lie rule blunt, treats every claim as guilty by default, and keeps the session hard-locked to the historical manual archive blocker unless it explicitly stops as blocked.
...
Validate the finished startup prompt note, checkpoint it cleanly, and use that note directly for future GPT-5.4 VS Code sessions on the first blocker lane.

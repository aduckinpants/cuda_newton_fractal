...
- [x] Phase 4 - wire the scenario driver into a mandatory public validation rail and contract proof chain so future UI slices cannot close without published-runtime proof, hostile-audit evidence, and live-window viewer-path witnesses where applicable.
...
- Done: `tests/runtime_harness.py` now owns a real live-window advanced-color drag surface too: Win32 pointer dragging plus runtime-side UI automation rect polling can target a published Color Pipeline control by id instead of guessed coordinates.
- Done: `ui_app/src/main.cpp`, `ui_app/src/viewer_cli.cpp`, `ui_app/src/viewer_cli.h`, `ui_app/src/viewer_state_init.cpp`, and `ui_app/src/color_pipeline_window.h` now expose the minimum truthful viewer seam for that witness: startup-open Color Pipeline flags, runtime JSON automation reports, forced-open automation rows, and per-control widget rect capture.
- Done: `tests/test_fractal_runtime_runtime_walk_viewer.py::test_runtime_walk_viewer_physical_color_pipeline_drag_changes_frame` now proves a physical published-runtime Color Pipeline drag that changes the rendered frame instead of faking the same state transition through headless-only draft mutation.
- Done: hostile audit of the expanded mandatory rail found and repaired two real programmable-color ownership defects that the new viewer witness forced into the open: successful draft apply was rebuilding the draft from live and renumbering row ids, and preserved drafts were not resynchronized when a CLI fractal override made them disallowed for the new family.
- Done: post-repair validation is green on the closure rails touched by this slice: `py -3.14 tools/viewer_host_run_logged_command.py --label "native helper tests" --log artifacts/verify_native_helper_tests.log -- cmd /c ui_app\build_tests_vsdevcmd.cmd`, `verify: runtime publish`, `verify: runtime ui harness`, `verify: runtime probe/session pytest`, `verify: runtime artifact tools`, and `py -3.14 tools/code_quality_audit.py --check-baseline --out artifacts/code_quality_report.json` all passed on the current slice.
...
- [done] Pass 22 - challenged whether the next advanced-color viewer witness was still faking interaction through headless-only state edits, added a real published-runtime physical Color Pipeline drag proof, and forced the runtime to expose real control geometry instead of guessed screen coordinates.
- [done] Pass 23 - challenged the mandatory runtime UI harness immediately after that witness landed, found two real programmable-color defects in draft ownership and family-override resynchronization, repaired both at the owning `color_pipeline_window.h` seam, and re-proved the full runtime profile plus mandatory UI harness rail.
...
- [done] A truthful live-window advanced-color witness needs two truths at once: a real pointer path and a real runtime-side control catalog. `tests/runtime_harness.py` plus the new automation-report seam now provide both, so viewer-path proofs can target the actual Color Pipeline widget rects by id.
- [done] The first expanded mandatory UI harness run exposed two real programmable-color bugs instead of fake-green coverage gaps: `ApplyColorPipelineDraftToLiveState` was resetting the draft from live and burning stable row ids after successful apply, and `SyncColorPipelineWindowFromLiveState` was preserving a saved draft even when a CLI fractal override made that draft disallowed for the new family.
- [done] The saved-draft resync proof is now explicit and green in `tests/test_fractal_runtime_explaino_escape_variants.py`, while the updated loaded-draft scenario witness also locks the already-normalized grading lane bookkeeping (`next_row_id == 15`, grading row id `14`) instead of the stale pre-normalization expectation.
- [done] One all-up rerun of `verify: runtime ui harness` transiently reported zero-diff animation failures in the sidecar, sweep, and runtime-walk replay witnesses immediately after the final color-pipeline fix, but the focused three-test subset and the immediate full rerun both passed on the same published runtime without further code change. Treat that as residual viewer-path flake risk, not a confirmed regression in the repaired color-pipeline seam.

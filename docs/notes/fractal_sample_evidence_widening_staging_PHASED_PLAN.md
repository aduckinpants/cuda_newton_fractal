...
- land one bounded second widened consumer on a downstream projection surface
- prove the existing widened payload is enough to drive a real projection-and-flow witness

Must prove:

- one second real widened consumer reads the existing widened evidence meaningfully
- legacy projection semantics still hold
- no speculative evidence field growth was needed

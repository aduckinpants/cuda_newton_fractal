...
- Color Pipeline typed Source-row applicators: Source rows now support `none`, `sdf_boundary_band`, `sdf_inside`, and `sdf_outside` applicator modes through UI-Salt metadata, runtime masks, state/capture/replay, and no-mouse proof.
- Mixed Source rows: supported renderer-backed lanes can blend SDF and non-SDF Source rows as scalar values in author order before Shape, Palette, and Grading; pure SDF CUDA paths remain preserved. `generic_equation_pack` and `sdf_pack_scene` mixed Source rows intentionally fail closed until they have renderer-backed non-SDF source-signal producers.
- Color Pipeline function picker/layout refinement: row Function selectors now render taxonomy-backed grouped headers while preserving option order, function ids, row semantics, and no-mouse control ids.
- Capture Finding fractal-state sidecar: Capture Finding archives now include `fractal-state.json` plus `finding.json.fractal_state_file` for review-focused active controls, while `state.json` remains the complete replay authority.
...
23. First SDF-native fractal lane. Shipped as `sdf_pack_scene`; built-in pack catalog seed, SDF drag mapping repair, typed Color Pipeline applicative glue, mixed Source rows for supported renderer-backed lanes, function picker/layout refinement, published-runtime pack staging, field capability reporting, and field-primary mixed Source denial proof are shipped. The next selected product step should optimize field generation/downsample before adding new SDF ops.

...
- Next selected product step: implement Color Pipeline function picker/layout refinement using existing UI-Salt taxonomy, compatibility, companion, applicator, and mixed Source-row metadata. Broader Color Pipeline UI redesign, more applicator semantics, full authored-pack catalog/authoring UX, new SDF ops, and additional SDF-native lanes remain separate later slices.

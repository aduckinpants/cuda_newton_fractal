...
- Blocker: must be fixed before claiming foundation closure. The manual `234919_563__explaino_inertial` capture reproduction failure remains a blocker after partial serialization repairs until a repair slice proves the archived frame re-renders from saved state or Adam explicitly accepts the old capture as unrecoverable historical data loss.
...
| Diagnostics serialization | Blocker for manual `explaino_inertial` reproduction | Shape, Palette, and Grading stacks save/load, and the manual `explaino_joy` fixture catches high-precision save-path truncation. The repair slice fixed split-color common-param reload and added forward `explaino_damping` persistence, but foundation closure is still blocked because the historical `234919_563__explaino_inertial` state re-renders as a 22-color orange frame instead of the archived fractal. | `ui_app/src/diagnostics_capture.cpp`, `ui_app/src/diagnostics_state_io.cpp`, `ui_app/tests/test_diagnostics_state_io.cpp`, `tests/test_fractal_runtime_manual_capture_repro.py`, `ck:f91b869f` |
| Capture/archive persistence | Blocked by manual `explaino_inertial` reproduction | Capture Finding carries advanced-color draft/stack state covered by helper tests, but closure cannot cite archive persistence as complete while a real manual finding stores a frame that does not reproduce from its archived state. The current evidence points at historical save-time data loss, not a green archive proof. | `ui_app/tests/test_finding_archive_actions.cpp`, `docs/notes/advanced_color_library_foundation_phase6_grading_runtime_authority_PHASED_PLAN.md`, `docs/notes/advanced_color_library_foundation_phase6_palette_blend_stack_PHASED_PLAN.md` |

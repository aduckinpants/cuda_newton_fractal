...
| Diagnostics serialization | Shipped for current forward/runtime boundary with historical-archive caveat | Shape, Palette, and Grading stacks save/load, and the manual `explaino_joy` fixture catches high-precision save-path truncation. The repair slice fixed split-color common-param reload and added forward `explaino_damping` persistence. The remaining `234919_563__explaino_inertial` mismatch is tracked as a separate historical compatibility issue rather than proof that current serialization is unshipped. | `ui_app/src/diagnostics_capture.cpp`, `ui_app/src/diagnostics_state_io.cpp`, `ui_app/tests/test_diagnostics_state_io.cpp`, `tests/test_fractal_runtime_manual_capture_repro.py`, `ck:f91b869f`, `ck:49038a61`, `ck:20260512-serializer-rail` |

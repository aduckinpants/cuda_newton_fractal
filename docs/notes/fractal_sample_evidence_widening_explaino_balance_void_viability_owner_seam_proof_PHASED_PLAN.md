# Fractal Sample Evidence Widening - ExplainO-BalanceVoid Viability And Owner-Seam Proof
...
- [done] Open and execute only the bounded ExplainO-BalanceVoid viability / owner-seam proof slice that follows the closed projection-and-flow witness.
- [done] Preserve `FractalSampleResult` and `SampleFractalPoints(...)` as the shipped legacy projection surfaces unless repo-grounded proof forces a narrower truthful transition.
- [done] Do not widen into generic advanced-color, broader Explaino-family implementation, renderer replacement, whole-engine generic refactor, meta-basin, operator-itinerary, DSL/program-space, or unrelated cleanup.
- [done] Prove whether the current widened payload is sufficient or whether richer evidence would actually be required before any BalanceVoid promotion claim.
- [done] Close only with commit, validation receipt, contract proof receipt, clean tree, and a committed-head stale-plan reread.

## Proof Ledger

- Bootstrap re-proved branch=`feature/fractal-sample-evidence-widening`, `HEAD=a4e29168cf991a36f22f9cae97fab48752c35abf`, clean tree, fresh session, and an active locked contract still pointing at the already closed projection-and-flow contract `fractal_sample_evidence_widening_projection_and_flow_witness`.
- Current single-orbit authority is still the shipped legacy surface in `ui_app/src/fractal_sample_result.h`, `ui_app/src/fractal_types.h`, `ui_app/src/fractal_sample_core.cu`, `ui_app/src/fractal_renderer.cu`, and `ui_app/src/runtime_walk_field_slime.cpp`; `SampleFractalPoints(...)` already projects from widened host sampling back to `FractalSampleResult`.
- Current widened evidence authority is still bounded to `sample_coord + legacy_result` in `ui_app/src/fractal_sample_result.h`, and `ui_app/src/fractal_sample_core.cu` still emits that payload without richer orbit traces, paired-state blobs, or renderer-specific ownership.
- `ui_app/src/fractal_sample_device.inl` is the only required owner seam currently using `balance_void`, `symmetry_tension`, and `field_curvature`; those values stay inside ExplainO-BalanceVoid iteration math and are not emitted through the widened evidence payload.
- `ui_app/src/fractal_family_rules.h`, `ui_app/src/schema_binding.cpp`, `ui_app/tests/test_schema_binding.cpp`, and `ui_app/tests/test_ui_schema.cpp` already prove the BalanceVoid axes are dedicated canonical Explaino parameter owners on the live `explaino_all` surface rather than hidden side channels or grading-only aliases.
- `ui_app/src/explaino_sidecar_model.cpp` derives the sidecar hypothesis space directly from the schema-driven `fractal.sample` catalog, and `ui_app/src/explaino_sidecar_measurement.cpp`, `ui_app/src/explaino_sidecar_lens.cpp`, and `ui_app/src/explaino_sidecar_window.cpp` consume that surface generically through applicable parameter rows rather than through a BalanceVoid-specific evidence contract.
- The RED head therefore still shows no real bounded BalanceVoid-specific widened-consumer seam: the widened consumer seams already landed are paired-state/counterfactual measurement plus projection-and-flow lens/window, and no current owner seam reads BalanceVoid-specific evidence fields beyond the generic param rows plus the existing `sample_coord + legacy_result` payload.
- Current repo proof points toward a stop-here / non-promotion answer unless a focused RED disproves it: BalanceVoid already owns parameter-surface rows, while sample-evidence widening has not exposed any distinct next owner seam that needs richer emitted evidence.
- Minimal blocker-clearing preflight is required before feature edits: the closed projection-and-flow contract does not scope this new plan/contract pair, while `viewer_host_begin_work_slice.py` and `viewer_host_prepare_slice.py` both require those checked-in files to exist before the lock can be replaced.
- Checked-in RED landed in the focused measurement compile: the first proof head demanded `BuildBalanceVoidParityCatalog()` and `BalanceVoidEvidenceHost`, and the current repo failed with missing-identifier compile errors, which proved no machine-backed BalanceVoid owner-seam or stop-here witness existed on the RED head.
- First GREEN now lives in `ui_app/tests/test_explaino_sidecar_measurement.cpp`: legacy `explaino_balance_void` and canonical `explaino_all` now build the same three BalanceVoid measurement rows on widened evidence, preserve zero `sample_coord` displacement, keep the legacy sample host unused, and prove the existing payload is sufficient through nested `legacy_result` deltas with no new fields.
- First outward owner-seam proof now lives in `ui_app/tests/test_explaino_sidecar_window.cpp`: the sidecar window path canonicalizes legacy `explaino_balance_void` to `explaino_all`, keeps the canonical selector plus the three BalanceVoid rows visible on the generic window surface, avoids the legacy sample host, preserves zero projection-flow bias, and keeps a non-zero window-side signal on the existing widened payload.
...
- [x] Pass 1 - prove the RED head still has no real bounded ExplainO-BalanceVoid widened-consumer seam and that projection-and-flow does not already count as BalanceVoid proof.

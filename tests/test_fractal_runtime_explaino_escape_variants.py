...
def test_explaino_all_neutral_defaults_match_explaino_published_runtime() -> None:
    ...
    explaino_all_state = explaino_all_capture["state"]
    assert explaino_all_state["fractal_type"] == "explaino_all"
    explaino_all_params = explaino_all_state["params"]
    assert isinstance(explaino_all_params, dict)
    assert explaino_all_params["ripple_amplitude"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["splice_offset"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["vortex_strength"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["tension_strength"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["balance_void"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["symmetry_tension"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_params["field_curvature"] == pytest.approx(0.0, abs=1e-6)
    assert explaino_all_capture["frame_hash"] == explaino_capture["frame_hash"], (
        "expected Explaino-all neutral defaults to collapse exactly to the published Explaino frame"
    )
...
@pytest.mark.parametrize(
    ("fractal_type", "cluster_radius_param", "default_value", "active_value", "neutral_matches_explaino"),
    [
...
def test_explaino_cluster_radius_runtime_classification(
    tmp_path: Path,
    fractal_type: str,
    cluster_radius_param: str,
    default_value: float,
    active_value: float,
    neutral_matches_explaino: bool,
) -> None:
    if sys.platform != "win32":
        pytest.skip("explaino_cluster_radius split runtime regression is Windows-only")
    ...
    assert carrier_state["fractal_type"] == fractal_type
    assert isinstance(carrier_params, dict)
    assert carrier_params[cluster_radius_param] == pytest.approx(default_value, abs=1e-6)
    ...
    assert carrier_capture["frame_hash"] != explaino_capture["frame_hash"], (
        f"expected {fractal_type} cluster-radius defaults to stay outside the canonical Explaino frame"
    )
    assert _mean_absolute_frame_delta(explaino_capture["frame_bytes"], carrier_capture["frame_bytes"]) > 0.0, (
        f"expected {fractal_type} cluster-radius defaults to produce a measurable published-frame delta"
    )

    active_state = json.loads(json.dumps(carrier_state))
    active_params = active_state["params"]
    assert isinstance(active_params, dict)
    active_params[cluster_radius_param] = active_value
    active_capture = _run_headless_capture(
        str(exe_path),
        "--load-state-json",
    ...
    assert active_capture["state"]["params"][cluster_radius_param] == pytest.approx(active_value, abs=1e-6)
    assert active_capture["frame_hash"] != carrier_capture["frame_hash"], (
        f"expected {fractal_type} cluster-radius activation to remain runtime-real"
    )
    assert _mean_absolute_frame_delta(carrier_capture["frame_bytes"], active_capture["frame_bytes"]) > 0.0, (
        f"expected {fractal_type} cluster-radius activation to change the published runtime frame"
    )

    neutral_state = json.loads(json.dumps(active_capture["state"]))
    neutral_params = neutral_state["params"]
    assert isinstance(neutral_params, dict)
    neutral_params[cluster_radius_param] = 0.0
    neutral_capture = _run_headless_capture(
        str(exe_path),
        "--load-state-json",
    ...
    assert neutral_capture["state"]["params"][cluster_radius_param] == pytest.approx(0.0, abs=1e-6)
    if neutral_matches_explaino:
        assert neutral_capture["frame_hash"] == explaino_capture["frame_hash"], (
            f"expected {fractal_type} neutral cluster-radius collapse to match the published Explaino frame"
        )
    else:
        assert neutral_capture["frame_hash"] != explaino_capture["frame_hash"], (
            f"expected {fractal_type} neutral cluster-radius state to stay outside the canonical Explaino baseline"
        )


def test_explaino_all_ignores_deferred_cluster_radius_param(tmp_path: Path) -> None:
    if sys.platform != "win32":
        pytest.skip("explaino_cluster_radius split runtime regression is Windows-only")
    ...
    assert canonical_capture["state"]["fractal_type"] == "explaino_all"
    assert canonical_capture["frame_hash"] == explaino_capture["frame_hash"], (
        "expected explaino_all to ignore deferred explaino_cluster_radius until it gains explicit canonical ownership"
    )
...
def test_explaino_headless_color_pipeline_scenario_can_switch_grading_row_to_neutral_finish(tmp_path: Path) -> None:
    if sys.platform != "win32":
        pytest.skip("Explaino advanced-color runtime regression is Windows-only")
    ...
    scenario_result = _run_headless_loaded_state_scenario(
        ...
        exe_path=exe_path,
        scenario=_HeadlessLoadedStateScenario(
            name="headless_color_pipeline_neutral_finish",
            state=draft_state,
            action_args=(
                "--color-pipeline-action",
                "select_function:grading:0:neutral_finish",
                "--color-pipeline-action",
                "set_param:grading:0:grade.exposure:number:1.25",
                "--color-pipeline-action",
                "set_param:grading:0:grade.saturation:number:0.75",
                "--color-pipeline-action",
                "set_param:grading:0:grade.contrast:number:1.5",
            ),
        ),
    )
    neutral_capture = scenario_result.scenario_capture

    neutral_params = neutral_capture["state"]["params"]
    assert isinstance(neutral_params, dict)
    assert neutral_params["color_grading"] == "neutral_default"
    assert neutral_params["exposure"] == pytest.approx(1.25, abs=1e-6)
    assert neutral_params["color_saturation"] == pytest.approx(0.75, abs=1e-6)
    assert neutral_params["color_contrast"] == pytest.approx(1.5, abs=1e-6)

    neutral_grading_row = _color_pipeline_first_row(neutral_capture["state"], "grading")
    assert neutral_grading_row["ui_row_id"] == 14
    assert neutral_grading_row["function_id"] == "neutral_finish"
    assert _color_pipeline_number_param(neutral_grading_row, "grade.exposure") == pytest.approx(1.25, abs=1e-6)
    assert _color_pipeline_number_param(neutral_grading_row, "grade.saturation") == pytest.approx(0.75, abs=1e-6)
    assert _color_pipeline_number_param(neutral_grading_row, "grade.contrast") == pytest.approx(1.5, abs=1e-6)

    assert neutral_capture["frame_hash"] != scenario_result.baseline_capture["frame_hash"], (
        "expected a headless advanced-color grading-row switch to neutral_finish to change the published runtime frame while persisting the grading draft row"
    )


def test_explaino_headless_color_pipeline_scenario_can_switch_grading_row_to_tone_map_finish(tmp_path: Path) -> None:
    if sys.platform != "win32":
        pytest.skip("Explaino advanced-color runtime regression is Windows-only")
    ...
    scenario_result = _run_headless_loaded_state_scenario(
        ...
        exe_path=exe_path,
        scenario=_HeadlessLoadedStateScenario(
            name="headless_color_pipeline_tone_map_finish",
            state=draft_state,
            action_args=(
                "--color-pipeline-action",
                "select_function:grading:0:tone_map_finish",
                "--color-pipeline-action",
                "set_param:grading:0:grade.exposure:number:1.35",
                "--color-pipeline-action",
                "set_param:grading:0:grade.saturation:number:0.75",
                "--color-pipeline-action",
                "set_param:grading:0:grade.contrast:number:1.6",
            ),
        ),
    )
    tone_map_capture = scenario_result.scenario_capture

    tone_map_params = tone_map_capture["state"]["params"]
    assert isinstance(tone_map_params, dict)
    assert tone_map_params["color_grading"] == "tone_map_default"
    assert tone_map_params["exposure"] == pytest.approx(1.35, abs=1e-6)
    assert tone_map_params["color_saturation"] == pytest.approx(0.75, abs=1e-6)
    assert tone_map_params["color_contrast"] == pytest.approx(1.6, abs=1e-6)

    tone_map_grading_row = _color_pipeline_first_row(tone_map_capture["state"], "grading")
    assert tone_map_grading_row["ui_row_id"] == 14
    assert tone_map_grading_row["function_id"] == "tone_map_finish"
    assert _color_pipeline_number_param(tone_map_grading_row, "grade.exposure") == pytest.approx(1.35, abs=1e-6)
    assert _color_pipeline_number_param(tone_map_grading_row, "grade.saturation") == pytest.approx(0.75, abs=1e-6)
    assert _color_pipeline_number_param(tone_map_grading_row, "grade.contrast") == pytest.approx(1.6, abs=1e-6)

    assert tone_map_capture["frame_hash"] != scenario_result.baseline_capture["frame_hash"], (
        "expected a headless advanced-color grading-row switch to tone_map_finish to change the published runtime frame while persisting the grading draft row"
    )
...
def test_explaino_headless_color_pipeline_scenario_can_switch_grading_row_to_balance_void_grade(tmp_path: Path) -> None:
    if sys.platform != "win32":
        pytest.skip("Explaino advanced-color runtime regression is Windows-only")
    ...
    scenario_result = _run_headless_loaded_state_scenario(
        ...
        exe_path=exe_path,
        scenario=_HeadlessLoadedStateScenario(
            name="headless_color_pipeline_balance_void_grade",
            state=draft_state,
            action_args=(
                "--color-pipeline-action",
    ...
    balance_void_capture = scenario_result.scenario_capture

    balance_void_params = balance_void_capture["state"]["params"]
    assert isinstance(balance_void_params, dict)
    assert balance_void_params["color_grading"] == "balance_void_default"
    assert balance_void_params["color_balance_void"] == pytest.approx(0.35, abs=1e-6)
    assert balance_void_params["color_chroma_tension"] == pytest.approx(0.6, abs=1e-6)
    assert balance_void_params["color_accent_bias"] == pytest.approx(-0.25, abs=1e-6)

    balance_void_grading_row = _color_pipeline_first_row(balance_void_capture["state"], "grading")
    assert balance_void_grading_row["ui_row_id"] == 14
    assert balance_void_grading_row["function_id"] == "balance_void_grade"
    assert _color_pipeline_number_param(balance_void_grading_row, "grade.balance_void") == pytest.approx(0.35, abs=1e-6)
    assert _color_pipeline_number_param(balance_void_grading_row, "grade.chroma_tension") == pytest.approx(0.6, abs=1e-6)
    assert _color_pipeline_number_param(balance_void_grading_row, "grade.accent_bias") == pytest.approx(-0.25, abs=1e-6)

    assert balance_void_capture["frame_hash"] != scenario_result.baseline_capture["frame_hash"], (
        "expected a headless advanced-color grading-row switch to balance_void_grade to change the published runtime frame while persisting the grading draft row"
    )

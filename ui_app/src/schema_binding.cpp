...
bool BindViewFloat(ViewState& view, const std::string& path, float** outPtr) {
    ...
    if (path == "fractal.view.explaino_phase" || path == "fractal.root_pattern.dynamics.phase") { *outPtr = &view.explaino_phase; return true; }
    ...
    if (path == "fractal.view.explaino_phase_strength" || path == "fractal.root_pattern.dynamics.phase_strength") { *outPtr = &view.explaino_phase_strength; return true; }
...
bool SetExplainoPatternBankEnum(BindingContext* ctx, const std::string& path, const std::string& id) {
    if (!ctx || !ctx->params) {
        return false;
    }
    if (path == "fractal.params.explaino_generated_root_layout" ||
    ...
    if (path == "fractal.params.explaino_root_field_pattern_ref") {
        return ParseAndAssignEnumId(
            id,
            &ctx->params->explaino_root_field_pattern_ref,
            TryParseExplainoRootPatternRefId);
    }
    return false;
}
...
std::string BindingContext::GetEnumId(const std::string& path) const {
    ...
    if (params && path == "fractal.params.explaino_root_field_pattern_ref") {
        return EnumIdOrEmpty(ExplainoRootPatternRefId(params->explaino_root_field_pattern_ref));
    }
...
bool BindingContext::SetEnumId(const std::string& path, const std::string& id) {
    ...
    if (SetExplainoPatternBankEnum(this, path, id)) {
        return true;
    }
    if (params && path == "fractal.params.explaino_root_sdf_h_source") {
...
bool BindingContext::GetBoolValue(const std::string& path, bool& out) const {
    ...
    if (path == "fractal.params.explaino_generated_root_count_active" ||
        path == "fractal.root_pattern.dynamics.generated_root_count_active") {
        out = view && params &&
...
bool ApplyDoubleSchemaDefault(const UISchemaControl& control, BindingContext& ctx, bool* ioDirty) {
    ...
    if ((control.binding.path == "fractal.params.explaino_seed" ||
         control.binding.path == "fractal.root_pattern.dynamics.seed") &&
        ctx.view && ctx.params) {

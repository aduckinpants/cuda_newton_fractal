...
static bool TryParseRuntimeWalkArgs(const std::vector<std::string>& args, ViewerCliArgs* out) {
    ...
    out->open_sdf_pack_panel_on_startup = HasArg(args, "--open-sdf-pack-panel");
    if (!TryStr(args, "--sdf-pack-json", &out->have_sdf_pack_json, &out->sdf_pack_json_path)) return false;
...
static bool TryParseMetadataHeadlessArgs(const std::vector<std::string>& args, ViewerCliArgs* out) {
    ...
    out->describe_active_fractal_model = HasArg(args, "--describe-active-fractal-model");
    ...
    if (!TryStr(args, "--describe-active-fractal-model-json", &out->have_describe_active_fractal_model_json, &out->describe_active_fractal_model_json_path)) return false;

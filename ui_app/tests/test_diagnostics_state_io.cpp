...
int main() {
  ...
  "render": {
    ...
    {
        const fs::path statePath = tempRoot / "explaino_balance_void_state.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "explaino_balance_void",
  "view": {
    "center_x": 0.0,
    "center_y": 0.0,
    "zoom": 1.0,
  ...
  "params": {
    ...
    "bell_coupling": 0.0,
    "balance_void": 0.42,
    "symmetry_tension": 0.18,
    "field_curvature": -0.35,
    "ripple_amplitude": 0.0,
    "splice_offset": 0.0,
    "vortex_strength": 0.0,
    "tension_strength": 0.0,
  ...
  "render": {
    ...
    "device_id": 0
        ...
        std::string error;
        if (!LoadDiagnosticsStateFile(statePath.string(), &view, &params, &render, &error)) {
            std::cerr << "ExplainO-BalanceVoid state should load: " << error << "\n";
            return 1;
        }
        ...
        if (!NearlyEqual(params.balance_void, 0.42f, 1.0e-6) ||
            !NearlyEqual(params.symmetry_tension, 0.18f, 1.0e-6) ||
            !NearlyEqual(params.field_curvature, -0.35f, 1.0e-6)) {
            std::cerr << "ExplainO-BalanceVoid family-axis params should round-trip through diagnostics state loading\n";
            return 1;
        }
        if (!NearlyEqual(params.ripple_amplitude, 0.0f, 1.0e-6) ||
            !NearlyEqual(params.splice_offset, 0.0f, 1.0e-6) ||
            !NearlyEqual(params.vortex_strength, 0.0f, 1.0e-6) ||
            !NearlyEqual(params.tension_strength, 0.0f, 1.0e-6)) {
            std::cerr << "ExplainO-BalanceVoid diagnostics load should keep legacy composed-variant axes neutral instead of widening into Explaino-all\n";
            return 1;
        }
        if (params.color_pipeline.signal != ColorSignal::root_index ||
            ...
            std::cerr << "ExplainO-BalanceVoid diagnostics load should preserve the neutral Explaino basin-default runtime tuple\n";
            return 1;
        }
    }
    {
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
    ...
    {
        const fs::path statePath = tempRoot / "v3_mixed_sdf_source_stack_base_signal_params.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "mandelbrot",
  ...
  "params": {
    ...
    "color_grading": "escape_default",
    "color_source_stack": [
      { "signal": "smooth_escape", "scale": 0.35, "bias": 0.1, "blend_weight": 1.0 },
      { "signal": "sdf_signed_distance", "scale": 0.05, "bias": 0.5, "blend_weight": 0.45, "sdf_gate": "boundary_band", "sdf_gate_width_px": 3.0 }
    ],
    "nova_alpha": 0.5,
    "phoenix_p_real": 0.0, "phoenix_p_imag": 0.0,
    "multibrot_power": 3,
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
        ...
        std::string error;
        if (!LoadDiagnosticsStateFile(statePath.string(), &v, &p, &r, &windowState, &error)) {
            std::cerr << "V3 mixed SDF/non-SDF source-stack load failed: " << error << "\n";
            return 1;
        }
        if (p.color_source_stack_count != 2 ||
            p.color_source_stack[0].signal != ColorSignal::smooth_escape ||
            !NearlyEqual(p.color_source_stack[0].params.scale, 0.35, 0.001) ||
            p.color_source_stack[1].signal != ColorSignal::sdf_signed_distance ||
            !NearlyEqual(p.color_source_stack[1].params.blend_weight, 0.45, 0.001) ||
            p.color_source_stack[1].params.sdf_gate != ColorPipelineSdfGateMode::boundary_band ||
            !NearlyEqual(p.color_source_stack[1].params.sdf_gate_width_px, 3.0, 0.001) ||
            p.color_pipeline.signal != ColorSignal::smooth_escape ||
            !NearlyEqual(p.color_smooth_escape_scale, 0.35, 0.001) ||
            !NearlyEqual(p.color_smooth_escape_bias, 0.1, 0.001) ||
            !windowState.live_snapshot.valid ||
            !windowState.live_snapshot.draft_import_supported ||
            windowState.live_snapshot.lanes.size() < 4 ||
            windowState.live_snapshot.lanes[0].rows.size() != 2 ||
            windowState.live_snapshot.lanes[0].rows[0].function_id != "smooth_escape_ramp" ||
            windowState.live_snapshot.lanes[0].rows[1].function_id != "sdf_signed_distance" ||
            !DraftRowHasEnumParam(windowState.live_snapshot.lanes[0].rows[1], "signal.sdf_gate", "boundary_band") ||
            !DraftRowHasNumberParam(windowState.live_snapshot.lanes[0].rows[1], "signal.sdf_gate_width_px", 3.0, 0.001)) {
            std::cerr << "Expected mixed SDF/non-SDF Source stacks to reload with first-row flat signal authority and live two-row Source lane\n";
            return 1;
        }
    }
    {
        const fs::path statePath = tempRoot / "v3_sdf_source_stack_params.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "mandelbrot",
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
        ...
        if (p.color_source_stack_count != 2 ||
            ...
            p.color_source_stack[1].params.sdf_field_downsample != 1 ||
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
    ...
    {
        const fs::path statePath = tempRoot / "v3_sdf_source_stack_bad_field_downsample.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "mandelbrot",
  ...
  "params": {
    ...
    "color_grading": "escape_default",
    "color_source_stack": [
      { "signal": "sdf_signed_distance", "scale": 0.05, "bias": 0.5, "blend_weight": 1.0, "sdf_field_downsample": 3 }
    ],
    "nova_alpha": 0.5,
    "phoenix_p_real": 0.0, "phoenix_p_imag": 0.0,
    "multibrot_power": 3,
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
        ...
        std::string error;
        if (LoadDiagnosticsStateFile(statePath.string(), &v, &p, &r, &error) ||
            error.find("sdf_field_downsample") == std::string::npos) {
            std::cerr << "Expected diagnostics state load to reject unsupported SDF Source field downsample\n";
            return 1;
        }
    }
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
...

    {
        ...
        if (!color_pipeline_core::TryParseAdvancedColorGradingFunctionId("neutral_finish", &neutralGrading)) {
            std::cerr << "Expected neutral_finish to parse as a shipped Grading row before diagnostics round-trip proof\n";
            return 1;
        }
        const fs::path statePath = tempRoot / "v3_neutral_finish_grading_stack.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "newton",
  ...
  "params": {
    "max_iter": 500, "epsilon": 1e-06, "exposure": 0.75,
    "poly_kind": 0,
    "coloring_mode": "smooth_escape",
    "color_signal": "smooth_escape",
    "color_shape": "identity",
    "color_palette": "cyclic_escape",
    "color_grading": "neutral_default",
    "color_grading_stack": [
      { "grading": "neutral_default", "exposure": 1.25, "saturation": 0.85, "contrast": 1.4 }
    ],
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
        ...
        std::string error;
        if (!LoadDiagnosticsStateFile(statePath.string(), &v, &p, &r, &error)) {
            std::cerr << "Neutral-finish diagnostics state load failed: " << error << "\n";
            return 1;
        }
        if (p.color_pipeline.grading != neutralGrading || p.color_grading_stack_count != 1 || p.color_grading_stack[0].grading != neutralGrading || !NearlyEqual(p.color_grading_stack[0].params.exposure, 1.25, 0.001) || !NearlyEqual(p.color_grading_stack[0].params.saturation, 0.85, 0.001) || !NearlyEqual(p.color_grading_stack[0].params.contrast, 1.4, 0.001) || !NearlyEqual(p.exposure, 1.25, 0.001) || !NearlyEqual(p.color_saturation, 0.85, 0.001) || !NearlyEqual(p.color_contrast, 1.4, 0.001)) {
            std::cerr << "Expected neutral_finish diagnostics state load to preserve grading stack params and the mirrored legacy grading owners\n";
            return 1;
        }
    }
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
...

    {
        ...
        if (!color_pipeline_core::TryParseAdvancedColorGradingFunctionId("grade_glow", &gradeGlowGrading)) {
            std::cerr << "Expected grade_glow to parse as a shipped Grading row before diagnostics round-trip proof\n";
            return 1;
        }
        const fs::path statePath = tempRoot / "v3_grade_glow_grading_stack.json";
        std::ofstream file(statePath, std::ios::out | std::ios::binary | std::ios::trunc);
        file << R"({
  "state_version": 3,
  "fractal_type": "newton",
  ...
  "params": {
    ...
    "color_shape": "identity",
    "color_palette": "cyclic_escape",
    "color_grading": "glow_default",
    "color_glow": 0.6,
    "color_grading_stack": [
      { "grading": "glow_default", "exposure": 1.2, "saturation": 0.8, "contrast": 1.5, "glow": 0.6 }
    ],
    "nova_alpha": 0.5, "phoenix_p_real": 0.0, "phoenix_p_imag": 0.0, "multibrot_power": 3,
    "explaino_seed": 0.0, "explaino_warp_strength": 0.0, "explaino_root_count": 0,
    "poly_coeffs": [-1, 0, 0, 1, 0]
  ...
  "render": { "width": 320, "height": 240, "block_size": 256, "device_id": 0 }
        ...
        std::string error;
        if (!LoadDiagnosticsStateFile(statePath.string(), &v, &p, &r, &error)) {
            std::cerr << "Grade-glow diagnostics state load failed: " << error << "\n";
            return 1;
        }
        if (p.color_pipeline.grading != gradeGlowGrading || p.color_grading_stack_count != 1 || p.color_grading_stack[0].grading != gradeGlowGrading || !NearlyEqual(p.color_grading_stack[0].params.exposure, 1.2, 0.001) || !NearlyEqual(p.color_grading_stack[0].params.saturation, 0.8, 0.001) || !NearlyEqual(p.color_grading_stack[0].params.contrast, 1.5, 0.001) || !NearlyEqual(p.color_grading_stack[0].params.glow, 0.6, 0.001) || !NearlyEqual(p.exposure, 1.2, 0.001) || !NearlyEqual(p.color_saturation, 0.8, 0.001) || !NearlyEqual(p.color_contrast, 1.5, 0.001) || !NearlyEqual(p.color_glow, 0.6, 0.001)) {
            std::cerr << "Expected grade_glow diagnostics state load to preserve grading stack params, the glow owner, and the mirrored legacy grading owners\n";
            return 1;
        }
    }

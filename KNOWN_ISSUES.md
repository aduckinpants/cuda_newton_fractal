...
- Field-generation stage telemetry, CUDA JFA buffer reuse, repeated median SDF witness reporting, measured CUDA SDF postprocess scratch-buffer reuse, row-local SDF field downsample UI/runtime authority, boundary-masked SDF Normal Angle Beauty, authored-pack live Color Pipeline/overlay field consumption, the first `sdf_pack_scene` selectable lane shell, the curated built-in SDF pack catalog seed, SDF drag camera mapping repair, typed Source-row applicators, mixed Source rows for supported renderer-backed lanes, grouped function picker/layout, Capture Finding `fractal-state.json` review sidecars, published-runtime built-in SDF pack staging, SDF field capability reporting, and field-primary mixed Source denial proof are shipped. The closed postprocess witness measured median improvements of `27%` to `62%` across identical SDF rows. The current SDF engine witness points next at field-generation/downsample optimization; full authored-pack catalog/authoring UX, new SDF ops, and broader SDF-native productization remain later slices, not another debounce-policy guess.
...
- Function picker/layout refinement is shipped. Keep full preset manager UI, full authored SDF pack catalog/authoring UX, new SDF ops, and additional SDF-native lanes as separate slices after the field-generation/downsample optimization pass.

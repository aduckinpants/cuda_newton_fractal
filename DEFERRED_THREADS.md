...
- Field-generation stage telemetry, CUDA JFA buffer reuse, repeated median SDF witness reporting, and measured CUDA SDF postprocess scratch-buffer reuse are shipped. The closed postprocess witness measured median improvements of `27%` to `62%`; the next unresolved field-resolution/composition seam is per-row/multi-field downsample authority before authored-pack UI or SDF-native lanes.

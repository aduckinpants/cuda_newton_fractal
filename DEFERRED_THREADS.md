...
| 7 | Color Pipeline composition follow-ups | Medium | High | UI-Salt backend authority, preset workflow truth, visible implementation-wording cleanup, SDF Normal Angle Beauty, typed Source-row applicators, and mixed SDF/non-SDF Source rows are shipped. The next bounded follow-up is function picker/layout refinement; more applicators, SDF pack authoring/catalog UX, new SDF ops, and SDF-native lanes remain later. |

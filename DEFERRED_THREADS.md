...
Short-term intent: the Wave 2 root-field consumer sprint is now bounded by `docs/notes/root_field_consumer_wave2_PHASED_PLAN.md` and stops after `root_field_debug_views`, `explaino_mandelbrot_root_trap`, and `explaino_magnet_root_well` classification. Do not turn that sprint into orbit traps, root-homotopy renderer work, trace-driven live rendering, new SDF force dynamics, Wave 3 medium enablers, or Wave 4 idea work.
...
| 8 | SDF performance follow-up after preview policy | Medium/High | High | Preview postprocess quality policy, CUDA SDF postprocess, adaptive field resolution, Lens Field v2 sign contrast, field-generation stage telemetry, CUDA JFA buffer reuse, repeated median SDF witness reporting, measured CUDA SDF postprocess scratch-buffer reuse, row-local SDF field downsample UI/runtime authority, boundary-masked normal-angle UX, Root SDF, generated N-root Root SDF, and Wave 2 root-field consumers are shipped. Return to field-generation/downsample optimization before adding new SDF ops or broader SDF-native families. |
...
- Field-generation stage telemetry, CUDA JFA buffer reuse, repeated median SDF witness reporting, measured CUDA SDF postprocess scratch-buffer reuse, row-local SDF field downsample UI/runtime authority, boundary-masked normal-angle UX, authored-pack live field consumption, the first `sdf_pack_scene` selectable lane shell, the curated built-in SDF pack catalog seed, SDF drag camera mapping repair, typed Source-row applicators, mixed SDF/non-SDF Source rows for supported renderer-backed lanes, grouped function picker/layout, Capture Finding `fractal-state.json` review sidecars, published-runtime built-in SDF pack staging, field capability reporting, and field-primary mixed Source denial proof are shipped. The closed postprocess witness measured median improvements of `27%` to `62%`; the next SDF engine optimization target is field generation/downsample, while more applicators, full authored-pack catalog/authoring UX, new SDF ops, and broader SDF-native families remain separate follow-ups. Mixed Source rows on `generic_equation_pack` and `sdf_pack_scene` remain fail-closed until those lanes have renderer-backed non-SDF source-signal producers.

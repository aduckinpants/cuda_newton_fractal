...
| 8 | SDF performance follow-up after preview policy | Medium/High | High | Preview postprocess quality policy, CUDA SDF postprocess, adaptive field resolution, Lens Field v2 sign contrast, field-generation stage telemetry, and CUDA JFA buffer reuse are shipped. The current witness points back to postprocess review for the representative matrix; per-row/multi-field downsample authority remains deferred. |
...
- Field-generation stage telemetry and CUDA JFA buffer reuse are shipped. The latest representative witness is stage-split and points back to postprocess review rather than another blind field-generation change before returning to authored-pack UI, SDF-native lanes, or per-row/multi-field downsample authority.
